...
fn main() {
    ...
    a + 1
    ";
    let code = "fun hoge() Int { return 1 }
    fun main() {
        hoge()
    }
    ";
...
fn sample_string11<'a>() {
    ...
    let tokens = lexer::str_to_tokens(s);
    let ast = parser::parse(&tokens);
    let stmts = vec![FuncDec(format!("hoge"), vec![(format!("aa"), IntTyp),(format!("bb"), IntTyp)], IntTyp, box Block(vec![Return(CallFunc(format!("+"), vec![IntExp(1), IntExp(1)]))]))];
    let target_ast = init_prog(stmts);
    assert_eq!(target_ast,ast)
}
...
fn sample_string7<'a>(){
    ...
    let tokens = lexer::str_to_tokens(s);
    let ast = parser::parse(&tokens);
    let stmts = vec![FuncDec(format!("huga"), vec![(format!("foo"), IntTyp)], IntTyp, box Block(vec![Return(CallFunc(format!("+"), vec![IntExp(1), IntExp(1)]))])), Return(CallFunc(format!("huga"), vec![IntExp(1)]))];
    let target_ast = init_prog(stmts);
    assert_eq!(target_ast,ast)
}
...
fn sample_string6<'a>(){
    ...
    let tokens = lexer::str_to_tokens(s);
    let ast = parser::parse(&tokens);
    let stmts = vec![FuncDec(format!("huga"), vec![(format!("foo"), NameTyp(format!("Hoge")))], IntTyp, box Block(vec![Return(CallFunc(format!("+"), vec![IntExp(1), IntExp(1)]))])), Return(CallFunc(format!("huga"), vec![IntExp(1)]))];
    let target_ast = init_prog(stmts);
    assert_eq!(target_ast,ast)
}
